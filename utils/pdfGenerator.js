...
/**
 * Vector Icon Helpers (Pure vector graphics for razor-sharp 300+ DPI print clarity)
 */
...
function drawGoldSealBadge(doc, cx, cy, radius = 20) {
  doc.save();
  // Outer gold rim with 3D gradient effect
  doc.circle(cx, cy, radius).lineWidth(1.6).strokeColor("#d97706").fillColor("#fffbeb").fillAndStroke();
  doc.circle(cx, cy, radius - 2.5).lineWidth(0.7).strokeColor("#fbbf24").stroke();
  // Inner document icon
  drawDocIcon(doc, cx - 4.5, cy - 5.5);
  doc.restore();
}
...
/**
 * Generates an ultra-sharp, pure vector PDF proposal & payment receipt
 * combining 100% razor-sharp typography, vector ribbons, and high-fidelity branding.
 */
function generateQuotationPDF(data = {}) {
  return new Promise((resolve, reject) => {
    try {
      ...
      // Brand Color Palette (Deep luxury navy & ochre gold)
      ...
      const paymentMethod = isPaid ? "UPI (9028833275@ptbl) / Bank" : "Pending Kickoff";
      ...
      // =============================================================
      // 0. LUXURY PAGE BORDER & CORNER ACCENTS (Pure Vector Sharpness)
      // =============================================================
      doc.save();
      // Outer subtle page border
      doc.roundedRect(12, 12, doc.page.width - 24, doc.page.height - 24, 8)
        .lineWidth(0.8).strokeColor("#cbd5e1");
      ...
      // =============================================================
      // 2. HEADER BRANDING & DUAL DOCUMENT IDENTITY (Vector Sharp)
      // =============================================================
      const logoSize = 62;
      if (fs.existsSync(logoPath)) {
        doc.save();
      ...
      // =============================================================
      // 8. LUXURY BOTTOM FOOTER (Vector Sharp)
      // =============================================================
      const footerY = 818;
      doc.save();
      doc.fillColor(purpleDark).fontSize(7.5).font("Helvetica-Bold")
